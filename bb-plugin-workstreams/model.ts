...
BB means changes to the BB application itself or its built-in features. Agent operations are grouped as tomdaleOS (never a separate "Agent operations" or "Agent manager" group) even when they run in or mention BB. Status-only summaries of all active workstreams are Cross-project coordination. A personal one-off asking "what is BB?" or "bb?" belongs to BB rather than Unclassified: the user is asking about BB. A narrow smoke test that checks an environment variable inside a BB thread, without naming another product, belongs to BB. Cross-project status summaries should not be combined with agent operations: agent instructions and skills, delegation and manager workflows, agent-manager tooling, BB thread-organizing automations, workstream status summaries. Vercel's automations product (vercel/api automations, apps/vercel-automations) is part of Vercel Agent. Repository guidance changes belong to the repository whose guidance is being edited (e.g. tomdaleOS); global cross-harness guidance belongs to Agent configuration. Scripts developed inside an interview project belong to that project, not a separate project named after the script. Workforest managing a checkout does not imply the code is Workforest. A broad manager status thread covering unrelated products is Cross-project coordination, not whichever product it mentions most.
Naming: the group is the product name alone. Never append or prepend descriptors such as plugin, addon, package, repo, provider, or a feature ("Sidebar Hierarchy" not "Sidebar Hierarchy Plugin"; "BB Recap" not "BB Recap plugin"; "fx" not "bb-plugin-fx-provider"; "BB" not "BB Pi Provider" or "BB - Okta auth"). Turn repository and package slugs into the product's name (engineering-full-stack-collab → Engineering Full-Stack Collab), dropping per-person, per-candidate, or per-fork suffixes. A monorepo or org path (e.g. owner/api) is not a product; name the product the change serves. Prefer the short recognizable project name, not a feature slug, path, package prefix, or combined historical title. Keep related but distinct products separate. Do not expand acronyms. Use explicit product identity across repositories when available; do not invent a more specific product surface. Use Unclassified only when identity cannot be established from the supplied context. If the supplied request asks a question about BB and provides no software context, "BB" is the project; if it asks a one-off question whose context is genuinely unknowable, use Unclassified. A product named in the conversation beats the repository or path name; only when no product is named anywhere, fall back to the checkout's repository instead of Unclassified.`;
